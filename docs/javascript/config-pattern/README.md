...
1. **Create a `config/` folder** in your project. Common section examples from a hypothetical project include:

`config/header.ts`
```ts
// Sometimes we need a unilingual site name for when i18n is not available
export const siteName = 'My Awesome Astro Site';
...
export const description = 'A wonderful site about neat things.';
```

`config/hosting.ts`
```ts
/// Repository edit links
export const user = process.env.APP_USER ?? 'my-username';
export const repo = process.env.APP_REPO ?? 'my-astro-site';
